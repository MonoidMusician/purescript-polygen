...
renderPolyBuilder l s = HH.div_ $ go poly c0 0 []
  where
    poly = unwrap (s ^. l)
    c0 = _CharCode (\a -> a + dimension' poly - 1) 'a'
    go a c i r = case uncons a of
      ...
      Just { head: b, tail: a' } ->
        ...
        in go a' c' (i+1) $
          [HH.label_ [cb, HH.text (var <> genx i)], op] <> r
